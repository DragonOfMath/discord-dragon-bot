...
module.exports = {
	...
	'e926': {
		...
		category: 'NSFW',
		info: 'Search something on e926.net (the safe-for-work alternative of e621.net), maximum of 5 tags. (The `order:random` specifier will be automatically added)',
		parameters: ['tag1','[tag2]','[tag3]','[tag4]','[tag5]'],
		permissions: 'inclusive',
		fn({client, args, userID, channelID, serverID}) {
			var blacklist = client.database.get('servers').get(serverID).blacklist;
		...
		subcommands: {
			'source': {
				...
				parameters: ['hash|imagelink'],
				fn({client, args, userID, channelID}) {
					let hash = args[0].match(/[0-9a-f]{32}/);
					if (!hash) {
						throw 'Invalid MD5 hash.';
					}
