	...
	events: {
		convert() {
			return this.data.conversions.map(function ({from,to,value,rate}) {
				let newValue = CONVERSION_TABLE[from](value);
				if (to == 'ft' && newValue % 1 > 0) {
					return `${value} ${from}${rate} ≈ ${Math.floor(newValue)}'${Math.floor(12 * (newValue % 1))}" ${rate}`;
				} else {
					return `${value} ${from}${rate} ≈ ${newValue.toFixed(2)} ${to}${rate}`;
				}
			}).join('\n');
		}
	}
